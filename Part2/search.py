...
def search_IDS(kitchen_items=[], goal_node=None,depth=1):
    reference_task_tree = []
    items_to_search = []
    items_to_search.append(goal_node.id)
    items_already_searched = []
    ...
    while depth>0:
        ...
        if len(items_to_search) > 0 and depth==0: 
            depth += 1
        else:
            break
    reference_task_tree.reverse()
...
def getIndex(value,motion_success_rates):
    return motion_success_rates.index(value)
               
#-----------------------------------------------------------------------------------------------------------
def get_max_successrate_motion(candidate_units):
        ...
        with open("motion.txt", "r") as motion_file:
            ...
            for line in motion_file:
                ...
                lines.append([motion,success_rate])
        for unit in candidate_units:
            target_node=foon_functional_units[unit].motion_node
            for l in lines:
                if target_node==l[0]:
                    success_rate=l[1]
            motion_success_rates.append(success_rate)
        if(len(candidate_units)>1):
            x=max(motion_success_rates)
            idx=getIndex(x,motion_success_rates)
        return candidate_units[idx]
      
#----------------------------------------------------------------------------------------------------------
def get_min_inputnodes(candidate_units):
    no_of_input_nodes=[]
    for unit in candidate_units:
        target_node=foon_functional_units[unit].input_nodes
        #no_of_input_nodes.append(len(target_node))
        ingredients_count = sum(len(node.ingredients) for node in target_node)
        no_of_input_nodes.append(len(target_node)+ingredients_count)
    y=min(no_of_input_nodes)
    idx=getIndex(y,no_of_input_nodes)
    return candidate_units[idx]

#----------------------------------------------------------------------------------------------------------
#Heuristic 
def search_heuristic(kitchen_items=[], goal_node=None,method=""):
    if goal_node == None:
    ...
    while len(items_to_search) > 0:
        ...
        if not check_if_exist_in_kitchen(kitchen_items, current_item):
            candidate_units = foon_object_to_FU_map[current_item_index]
            if(method=="Heuristic1"):
                selected_candidate_idx = get_max_successrate_motion(candidate_units)
            if(method=="Heuristic2"):
                selected_candidate_idx= get_min_inputnodes(candidate_units)
            # if an fu is already taken, do not process it again
            if selected_candidate_idx in reference_task_tree:
                continue
    ...
    for node in goal_nodes:
        ...
        for object in foon_object_nodes:
            if object.check_object_equal(node_object):
                flag=1
                output_task_tree = search_heuristic(kitchen_items, object,method="Heuristic1")
                ...
                output_task_tree = search_heuristic(kitchen_items, object, method="Heuristic2")
                save_paths_to_file(output_task_tree,
